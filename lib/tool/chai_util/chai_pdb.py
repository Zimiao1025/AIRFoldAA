...
from chai_lab.data.dataset.all_atom_feature_context import (
    MAX_MSA_DEPTH, MAX_NUM_TEMPLATES, AllAtomFeatureContext)
...
from chai_lab.data.features.generators.missing_chain_contact import \
    MissingChainContact
from chai_lab.data.features.generators.msa import (IsPairedMSAGenerator,
...
from chai_lab.data.features.generators.templates import (
    TemplateDistogramGenerator, TemplateMaskGenerator,
    TemplateResTypeGenerator, TemplateUnitVectorGenerator)
...
def load_local_exported(local_path: Path, device: torch.device) -> torch.nn.Module:
    # local_path = chai1_component(comp_key)
    exported_program = torch.export.load(local_path)
    return exported_program.module().to(device)
...
def run_folding_on_context(
    ...
    *,
    ...
    for idx in range(num_diffn_samples):
        ...
        print(f"Score={aggregate_score:.3f}, writing output to {pdb_out_path}   ")
