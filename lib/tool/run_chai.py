...
from lib.tool.chai_util.chai_cif import run_inference
...
def chai_main(args):
    ...
    for i, chain in enumerate(chains):
        ...
        if chain.entity_data.entity_type == EntityType.PROTEIN:
            pqt_path = Path(args.msa_dir + "/" + expected_basename(chain.entity_data.sequence))
            df = a3m_to_aligned_dataframe(a3m_path=Path(args.a3m_paths[i]), source_database=MSADataSource.UNIREF90)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--a3m_paths", type=str, nargs="*")
